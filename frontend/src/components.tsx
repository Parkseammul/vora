...
export function RevisionModal({ nodeKey, onClose, onRevisionCompleted }: { nodeKey: NodeKey; onClose: () => void; onRevisionCompleted?: () => void }) {
  ...
  const submit = async () => { if (!text.trim()) { setError("수정 요청을 입력해주세요."); return; } setBusy(true); setError(""); try { const result = await api.revise(workflowExecutionId, nodeKey, text); await refetch(); if (result.restart_node !== nodeKey) setRestart(result.restart_node); else { onRevisionCompleted?.(); onClose(); } } catch (e) { setError(e instanceof Error ? e.message : "수정 요청에 실패했습니다."); } finally { setBusy(false); } };
...
export function ApprovalActions({ nodeKey, onRevisionCompleted }: { nodeKey: NodeKey; onRevisionCompleted?: () => void }) {
